...
public:
	...
	void PushBack(const T& p_refData)
	{
		...
		{
            std::cout<<"out of max size"<<std::endl;
	...
	T& Back(const int32_t& p_nTimeOut=-1) throw(std::string)
	{
		std::unique_lock<std::mutex> lock(m_Mutex);
		if(p_nTimeOut<0)
			m_Cv.wait(lock, [&]() {return !m_dqData.empty();});
	...
	T& Front(const int32_t& p_nTimeOut=-1) throw(std::string)
	{
		std::unique_lock<std::mutex> lock(m_Mutex);
		if(p_nTimeOut<0)
			m_Cv.wait(lock, [&]() {return !m_dqData.empty();});
	...
	//定义判断元素是否满足相关条件的函数原型
    typedef bool (*Condition)(const T& p_constRef);
	...
	//清空并释放本队列及所有元素空间
	void Release() throw(std::string)//指定异常参数表，则只能抛出指定类型的异常
	{
		std::unique_lock<std::mutex> lock(m_Mutex);
